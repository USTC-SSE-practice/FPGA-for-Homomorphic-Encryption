...
typedef struct {
    ...
    XPaillier_fl_kernel_uio_map maps[ MAX_UIO_MAPS ];
} XPaillier_fl_kernel_uio_info;

/***************** Variable Definitions **************************************/
static XPaillier_fl_kernel_uio_info uio_info;
...
static int uio_info_read_name(XPaillier_fl_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPaillier_fl_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPaillier_fl_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPaillier_fl_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPaillier_fl_kernel_Initialize(XPaillier_fl_kernel *InstancePtr, const char* InstanceName) {
	XPaillier_fl_kernel_uio_info *InfoPtr = &uio_info;
